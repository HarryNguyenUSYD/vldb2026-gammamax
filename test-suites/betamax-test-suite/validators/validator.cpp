...
bool is_isbn(std::string_view value) {
    ...
    return position + 1 == value.size() &&
           (std::isdigit(static_cast<unsigned char>(value[position])) || value[position] == 'X');
}
...
int main(int argc, char** argv) {
    ...
    bool accepted = false;
    if (name == "validate_date") accepted = is_date(input);
    else if (name == "validate_time") accepted = is_time(input);
    else if (name == "validate_url") accepted = is_url(input);
    else if (name == "validate_isbn") accepted = is_isbn(input);
    else if (name == "validate_ipv4") accepted = is_ipv4(input);
    else if (name == "validate_ipv6") accepted = is_ipv6(input);
    else return 2;
    return accepted ? 0 : 1;
}

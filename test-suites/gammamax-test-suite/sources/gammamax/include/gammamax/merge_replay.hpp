...
inline ReplayResult replay_merges(const Automaton& base,
    ...
    Automaton execution = partition.materialize(base);
    return {std::move(execution), std::move(partition),
            std::move(valid_history), conflict};
}

...
inline Automaton reject_exact_string(const Automaton& source,
    ...
    if (!source.accepts(value)) return source;

    const std::vector<StateId> source_path=source.path(value);
    ...
    for (std::size_t position=0;position<source_path.size();++position) {
        const bool accepting=position!=value.size() &&
                             source.state(source_path[position]).accepting;
        prefix_states.push_back(result.add_state(accepting));
    }

    for (std::size_t position=0;position<source_path.size();++position) {
        auto& transitions=result.state(prefix_states[position]).transitions;
        for (const auto& [symbol,destination]:source.state(source_path[position]).transitions)
            transitions.emplace(symbol,source.resolve(destination));
        if (position<value.size())
            transitions[static_cast<unsigned char>(value[position])]=
                prefix_states[position+1];
    }
    result.set_start_state(prefix_states.front());
    return result;
}

// Run paper-style single-result C/H RSR repeatedly. After each result, remove
// exactly that string from a temporary DFA so the next run must return a
// different minimum-edit candidate. The learned grammar is never modified.
inline std::vector<RepairCandidate> rsr_repairs(
    ...
    for (const auto& value:excluded)
        working=reject_exact_string(working,value);
    ...
    for (std::size_t run=0;run<rsr_batch_size;++run) {
        ...
        const std::string emitted=repair->value;
        repairs.push_back(std::move(*repair));
        working=reject_exact_string(working,emitted);
    }

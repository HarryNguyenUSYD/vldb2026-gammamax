...
inline std::string serialize_result(const ProgramResult& result) {
    std::uint64_t rsr_candidates_generated=0;
    std::size_t rsr_max_candidates_in_call=0;
    std::uint64_t rsr_calls_with_multiple_candidates=0;
    bool rsr_enumeration_truncated=false;
    nlohmann::json rsr_iterations=nlohmann::json::array();
    for (const auto& iteration:result.measurements.rsr_iterations) {
        rsr_candidates_generated+=iteration.unique_candidates;
        rsr_max_candidates_in_call=std::max(
            rsr_max_candidates_in_call,iteration.unique_candidates);
        if (iteration.unique_candidates>1) ++rsr_calls_with_multiple_candidates;
        if (!iteration.enumeration_complete) rsr_enumeration_truncated=true;
        rsr_iterations.push_back({
            {"minimum_edit_cost",iteration.minimum_edit_cost},
            {"unique_candidates",iteration.unique_candidates},
            {"candidates_after_ngrams",iteration.candidates_after_ngrams},
            {"enumeration_complete",iteration.enumeration_complete},
        });
    }
    nlohmann::json output = {
        ...
        {"rsr_total_calls", result.measurements.rsr_iterations.size()},
        ...
        {"rsr_iterations", std::move(rsr_iterations)},
    };
    return output.dump();
}

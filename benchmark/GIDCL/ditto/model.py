...
def get_device():
    """Use CPU when requested and avoid assuming CUDA is installed."""
    requested = os.getenv("GIDCL_DEVICE", "auto").lower()
    if requested == "cpu":
        return "cpu"
    if requested not in {"auto", "cuda"}:
        raise ValueError("GIDCL_DEVICE must be 'auto', 'cpu', or 'cuda'.")
    if requested == "cuda" and not torch.cuda.is_available():
        raise RuntimeError("GIDCL_DEVICE=cuda, but CUDA is not available.")
    return "cuda" if torch.cuda.is_available() else "cpu"


def use_fp16(hp, device):
    enabled = bool(getattr(hp, "fp16", False)) and device == "cuda"
    if enabled and amp is None:
        raise RuntimeError("FP16 detector training requires NVIDIA Apex.")
    return enabled
...
def load_model( path, lm, use_gpu, fp16=True):
    ...
    if fp16 and device == 'cuda':
        if amp is None:
            raise RuntimeError("FP16 detector loading requires NVIDIA Apex.")
...
def simple_train_update(trainset, validset, testset, hp,optimizer, model_input=False):
    ...
    if(isinstance(model_input,DittoModel)):
        ...
        model,optimizer = load_model_update(
            model_input, 'roberta-base', use_gpu=(device == 'cuda'),
            lr=hp.lr, fp16=use_fp16(hp, device)
        )
        # model = model.cuda()
    else:
        model = DittoModel(device=device,

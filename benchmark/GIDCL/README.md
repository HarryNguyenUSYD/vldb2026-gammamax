...
The portable path runs the RoBERTa detector on CPU and sends correction prompts
to an OpenAI-compatible API. It does not require CUDA, Apex, bitsandbytes,
FlashAttention, or vLLM.

```bash
conda create -n gidcl-cpu python=3.10 -y
conda activate gidcl-cpu
pip install -r requirements-macos-cpu.txt

export GIDCL_DEVICE=cpu
export OPENAI_API_KEY="your-key"
export OPENAI_API_BASE="https://api.openai.com/v1"
export GIDCL_MODEL="gpt-4o-mini"
```

Credentials follow the same environment-only design as ZeroEC:

- `OPENAI_API_KEY` is required.
- `OPENAI_API_BASE` defaults to `https://api.openai.com/v1`.
- `GIDCL_MODEL` defaults to `gpt-4o-mini`.

Run `detector_train.ipynb` after changing its dataset placeholder to the target
dataset. The checked-in notebook disables FP16, and `ditto/model.py` honors
`GIDCL_DEVICE=cpu` without requiring NVIDIA Apex.

Run correction inference from the `GIDCL` directory:

```bash
python vllm_inference_api.py \
  --backend openai \
  --test_file GEIL_Data/beers/correction/test.json \
  --json \
  --output inference/beers-test-output.csv
```

The API backend uses prepared GIDCL prompts but not a local LoRA checkpoint.
This is the minimal Mac-compatible execution path. The original paper path,
using a fine-tuned local correction model through vLLM, still requires a
supported accelerator host. Preserve that path with `--backend vllm`.

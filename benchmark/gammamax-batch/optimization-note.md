...
## 11. Multiple RSR calls per refinement iteration

Paper-style C/H RSR returns one minimum-edit repair. The implementation calls it
repeatedly to obtain `rsr_batch_size` distinct candidates during one refinement
iteration.

After each RSR result, gammaMax constructs a temporary DFA whose language is the
working language minus exactly that result. It then runs the unchanged
single-result RSR procedure again. The learned grammar itself is not modified.

This is intentionally repeated single-result RSR, rather than a separate
k-shortest-path algorithm. It preserves the semantics and tie behaviour of the
paper-style routine while exposing several alternatives to ranking and oracle
validation.

The benchmark-selected setting is `rsr_batch_size=8`.

Relevant code: `rsr_repair()`, `reject_exact_string()`, and `rsr_repairs()` in
`rsr.hpp`.
...
## 13. Separate RSR and n-gram batch sizes

Candidate generation and oracle querying use two independently configurable
batch sizes:

- `rsr_batch_size`: number of distinct repairs generated by repeated RSR;
- `ngrams_batch_size`: number retained after n-gram ranking and sent to the
  oracle.

The required invariant is:

```text
1 <= ngrams_batch_size <= rsr_batch_size
```

This creates a generate-many/query-few pipeline. RSR can explore several repair
alternatives while expensive external oracle calls are restricted to the most
promising subset. The benchmark-selected values are 8 and 1 respectively.
...
| `repair.rsr_batch_size` | Number of distinct RSR results generated | Not accepted |
| `repair.ngrams_batch_size` | Number of ranked results queried | Not accepted |
...
validates their shape and numeric ranges, and emits one machine-readable JSON
result object. Legacy single-cell input remains ASCII-only. Batch `cells`
accept UTF-8 because shared benchmark datasets contain non-ASCII text.
...
## 24. Oracle classification before batch learning

Batch input supplies every cell from one column in a single `cells` array.
Before constructing the PTA, gammaMax queries the column oracle once for every
cell. Accepted cells become positive examples. Rejected cells become known
negatives and repair inputs. The normal reusable batch session begins after
this scan. Because repair inputs are already known to be rejected, their old
initial oracle check is skipped; candidate repairs are still queried normally.

The executable processes one column per invocation because one configured
oracle executable defines one column's language. Dataset-level orchestration
invokes it separately for every column. Classification time and counts are
reported under `oracle_scan`.

Relevant code: `json_io.hpp`, `batch_session.hpp`, and `main.cpp`.

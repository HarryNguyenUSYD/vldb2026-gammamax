...
inline Config parse_config_value(const nlohmann::json& root) {
    ...
    r.max_positive_examples=limits.contains("max_positive_examples")
        ? boundary_value<std::size_t>(limits,"max_positive_examples","config.limits")
        : 500;
    ...
    r.max_queue_size=limits.contains("max_queue_size")
        ...
        : std::numeric_limits<std::size_t>::max();
    r.cell_timeout_seconds=root.contains("cell_timeout_seconds")
        ? unsigned_value<std::size_t>(root,"cell_timeout_seconds","config") : 60;
    if (!r.rsr_batch_size || !r.ngrams_batch_size ||
        r.ngrams_batch_size>r.rsr_batch_size ||
        !r.max_candidate_length || !r.max_iterations ||
        !r.max_total_oracle_calls || !r.max_positive_examples ||
        !r.max_states || !r.max_queue_size ||
        !r.cell_timeout_seconds)

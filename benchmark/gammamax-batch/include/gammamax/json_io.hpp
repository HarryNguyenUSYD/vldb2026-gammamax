...
inline std::vector<std::string> parse_string_array(const nlohmann::json& value,
    ...
    for (const auto& item : value) {
        ...
        if (require_ascii && !is_ascii(text)) {
...
inline BatchInputData parse_batch_input_value(const nlohmann::json& root) {
    ...
    if (!root.contains("cells"))
        throw std::runtime_error("input requires cells");
    BatchInputData result;
    result.cells = parse_string_array(root.at("cells"), "input.cells", false);
    if (result.cells.empty())
        throw std::runtime_error("input.cells must not be empty");
    return result;
}

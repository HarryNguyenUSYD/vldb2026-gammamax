...
int main() {
    ...
    auto batch_input=parse_batch_input_value(nlohmann::json{{"cells",{"ab","ad","ac"}}});
    assert(batch_input.cells==std::vector<std::string>({"ab","ad","ac"}));
    bool old_batch_shape_rejected=false;
    try {
        (void)parse_batch_input_value(nlohmann::json{
            {"positive_examples",{"ab"}},{"corrupt_strings",{"ad"}}});
    } catch (const std::runtime_error&) { old_batch_shape_rejected=true; }
    assert(old_batch_shape_rejected);
    ...
    assert(config.max_positive_examples==500);
    ...
    // Batch preprocessing preserves duplicate caller input while the
    // reusable PTA is built from unique values only. A cell timeout does not
    // prevent the same session from repairing the next cell.
    config.cell_timeout_seconds=1;
    config.max_positive_examples=1;
    bool force_timeout=true;
    std::size_t clock_calls=0;
    const auto epoch=BatchSession::Clock::time_point{};
    BatchSession session({"ab","ab","ac"},{},config,[&] {
        if (!force_timeout) return epoch;
        return epoch+std::chrono::seconds(clock_calls++ ? 2 : 0);
    });
    assert(session.shared_measurements().input_positive_count==3);
    assert(session.shared_measurements().unique_positive_count==2);
    assert(session.shared_measurements().used_positive_count==1);
    assert(session.shared_measurements().ignored_positive_count==1);
    AlgorithmMeasurements timed_measurements;
    std::uint64_t timed_ns=0;
    bool timed_out=false;
    try { (void)session.repair("ad",oracle,timed_measurements,timed_ns); }
    catch (const CellTimeout&) { timed_out=true; }
    assert(timed_out);
    force_timeout=false;
    AlgorithmMeasurements next_measurements;
    std::uint64_t next_ns=0;
    const auto next=session.repair("ad",oracle,next_measurements,next_ns);
    assert(next=="ab");
    std::cout << "ok\n";
}

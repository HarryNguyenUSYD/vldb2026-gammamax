...
# Shared safeguards for every OpenAI-compatible API call. Defaults deliberately
# favor a slow, reliable run; override them only after checking account limits.
OPENAI_MAX_WORKERS = max(1, int(os.getenv("ZEROEC_OPENAI_MAX_WORKERS", "1")))
OPENAI_MAX_ATTEMPTS = max(1, int(os.getenv("ZEROEC_OPENAI_MAX_ATTEMPTS", "3")))
OPENAI_MAX_REQUESTS = max(1, int(os.getenv("ZEROEC_OPENAI_MAX_REQUESTS", "2000")))
OPENAI_MIN_REQUEST_INTERVAL_SECONDS = max(
    0.0, float(os.getenv("ZEROEC_OPENAI_MIN_REQUEST_INTERVAL_SECONDS", "2.0"))
)
OPENAI_RETRY_BASE_SECONDS = max(
    0.1, float(os.getenv("ZEROEC_OPENAI_RETRY_BASE_SECONDS", "2.0"))
)
OPENAI_RETRY_MAX_SECONDS = max(
    OPENAI_RETRY_BASE_SECONDS,
    float(os.getenv("ZEROEC_OPENAI_RETRY_MAX_SECONDS", "60.0")),
)

_openai_rate_lock = threading.Lock()
_openai_next_request_time = 0.0
_openai_request_count = 0
_token_count_lock = threading.Lock()


def _wait_for_openai_request_slot():
    """Globally pace requests and enforce a hard per-process request cap."""
    global _openai_next_request_time, _openai_request_count

    with _openai_rate_lock:
        if _openai_request_count >= OPENAI_MAX_REQUESTS:
            raise RuntimeError(
                f"OpenAI request cap reached ({OPENAI_MAX_REQUESTS}); "
                "increase ZEROEC_OPENAI_MAX_REQUESTS to continue"
            )

        now = time.monotonic()
        scheduled = max(now, _openai_next_request_time)
        _openai_next_request_time = scheduled + OPENAI_MIN_REQUEST_INTERVAL_SECONDS
...
def invoke_with_openai_safeguards(runnable, inputs, operation, transform=None):
    """Invoke an LLM with global pacing and bounded exponential backoff."""
    last_error = None
    for attempt in range(1, OPENAI_MAX_ATTEMPTS + 1):
        ...
        try:
            ...
            rate_limited = (
                "429" in str(error) or "rate_limit_exceeded" in str(error)
            )
            backoff = min(
                OPENAI_RETRY_BASE_SECONDS * (2 ** (attempt - 1)),
                OPENAI_RETRY_MAX_SECONDS,
            )
            if rate_limited:
                backoff = min(
                    max(backoff, OPENAI_MIN_REQUEST_INTERVAL_SECONDS),
                    OPENAI_RETRY_MAX_SECONDS,
                )
            backoff += random.random()
            print(
                f"{operation} failed (attempt {attempt}/{OPENAI_MAX_ATTEMPTS}); "
                f"retrying in {backoff:.1f}s: {error}"
            )
            time.sleep(backoff)

    raise RuntimeError(
        f"{operation} failed after {OPENAI_MAX_ATTEMPTS} attempts"
    ) from last_error
...
def get_auto_cot(repair_list, column, retriever, CoE, indices, detection_filtered, rep_error_info: dict, sp_examps):
    ...
    def parse_auto_cot(response):
        ...
        try:
            ...
            return ast.literal_eval(repair_result)
        except Exception as error:
            raise ValueError("Auto-CoT returned invalid structured output") from error

    repair_result_list = invoke_with_openai_safeguards(
        chain_auto_cot,
...
def code_generation(train_data:dict,column:str,codes:dict):
    ...
    raw_code = invoke_with_openai_safeguards(
        chain_code_generation,
        {'examples': examples},
        f"code generation for {column}",
...
def FD_generation(train_data:dict,column:str,FDs:dict):
    ...
    chain_fd_generation = (prompt_code_generation | llm_fd_generation)

    def parse_fd(response):
        fd_raw = response.content
        try:
            fd_raw = re.sub(r'```json', '', fd_raw)
            fd_raw = re.sub(r'```', '', fd_raw)
            return ast.literal_eval(fd_raw)
        except Exception as error:
            raise ValueError("FD generation returned invalid structured output") from error

    fd_dict = invoke_with_openai_safeguards(
        chain_fd_generation,
        {'examples': examples},
        f"FD generation for {column}",
        parse_fd,
    )
    FDs[column] = fd_dict
    pass
...
def repair_value(dirty_tuple, column, dirty_value, index_row, index_col, chain):
    ...
    inputs = {'Dirty_Tuple': dirty_tuple_filtered_str,
              'Erroneous_value': dirty_value_str,
              'Relevant_clean_tuples': relevant_clean_tuples}
    promt = prompt_dict[column].invoke(inputs)
    try:
        repair_result = invoke_with_openai_safeguards(
            chain,
            inputs,
            f"cell repair for row {index_row}, column {column}",
        )
...
class myEmbeddings(Embeddings):
    ...
    def __init__(self, modelPath):
        self.model = FastSentenceTransformer(modelPath, device="cpu", quantize=True)
...
if __name__ == "__main__":
    ...
    # param-prompts_path
    system_message_path = 'prompt_templates/SystemMessage-2.txt'
    human_message_path = 'prompt_templates/HumanMessage.txt'
    general_examples_path = 'prompt_templates/examples.txt'
    system_message_auto_cot_path = 'prompt_templates/SystemMessage_for_AutoCoT_with_error_type.txt'
    examples_auto_cot_path = 'prompt_templates/examples_for_AutoCoT_with_error_type.txt'
    ...
    system_message_data_augmentation_path = 'prompt_templates/SystemMessage_data_augmentation.txt'
    human_message_data_augmentation_path = 'prompt_templates/HumanMessage_data_augmentation.txt'
    ...
    MODEL_NAME = os.getenv("ZEROEC_MODEL", "gpt-4o-mini")
    ...
    MODEL_NAME_auto_cot = MODEL_NAME
    # The base URL for the API endpoint in AutoCoT
    OPENAI_API_BASE_auto_cot = OPENAI_API_BASE
    # The API key for authentication in AutoCoT
    OPENAI_API_KEY_auto_cot = OPENAI_API_KEY
    ...
    MODEL_NAME_fd_generation = MODEL_NAME
    # The base URL for the API endpoint in fd generation
    OPENAI_API_BASE_fd_generation = OPENAI_API_BASE
    # The API key for authentication in fd generation
    OPENAI_API_KEY_fd_generation = OPENAI_API_KEY
    ...
    print(
        "OpenAI safeguards: "
        f"workers={OPENAI_MAX_WORKERS}, "
        f"min_interval={OPENAI_MIN_REQUEST_INTERVAL_SECONDS}s, "
        f"max_attempts={OPENAI_MAX_ATTEMPTS}, "
        f"max_requests={OPENAI_MAX_REQUESTS}"
    )
    ...
    llm = ChatOpenAI(
        ...
        request_timeout=60,
        max_retries=0,  # Centralized retry policy is applied by invoke_with_openai_safeguards.
    )
    ...
    with open(os.path.join(output_path, 'output.txt'), 'a', encoding='utf-8') as f_output:
        ...
        f_output.write(f"OpenAI requests attempted: {_openai_request_count}\n")

...
inline BatchInputData parse_batch_input_value(const nlohmann::json& root) {
    require_object(root, "input");
    if (!root.contains("positive_examples") || !root.contains("corrupt_strings"))
        throw std::runtime_error("input requires positive_examples and corrupt_strings");
    BatchInputData result;
    result.positive_examples =
        parse_string_array(root.at("positive_examples"), "input.positive_examples");
    if (root.contains("negative_examples")) result.negative_examples =
        parse_string_array(root.at("negative_examples"), "input.negative_examples");
    result.corrupt_strings =
        parse_string_array(root.at("corrupt_strings"), "input.corrupt_strings");
    if (result.positive_examples.empty())
        throw std::runtime_error("input.positive_examples must not be empty");
    if (result.corrupt_strings.empty())
        throw std::runtime_error("input.corrupt_strings must not be empty");
    return result;
}

inline BatchInputData read_batch_input_json(const std::filesystem::path& path) {
    return parse_batch_input_value(read_json_file(path));
}

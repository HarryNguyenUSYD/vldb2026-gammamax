...
inline std::string gamma_max(const InputData& input,const Config& config,Oracle& oracle,
    ...
    for (std::size_t iteration=0;iteration<config.max_iterations;++iteration) {
        ...
        auto& rsr_measurement=measurements->rsr_iterations.back();
        auto repairs=rsr_repairs_in_place(
            working,input.corrupt_string,model,config.ngrams_batch_size,
            config.max_candidate_length,config.max_rsr_candidates,
            config.max_queue_size,known,
            &measurements->ngrams_execution_time_ns,&rsr_measurement);
